...
- Verify generated apps have a `design_intent_profile` before UI generation or UI review. Flag generic, emotionally mismatched, inaccessible, or audience-blind UI when target audience, user sophistication, trust needs, visual style preference, accessibility needs, or things to avoid are missing.
...
- Verify a `pending_check_resolution` artifact before accepting review-ready claims when an external status remains pending. Block failed checks, missing required checks, and required pending checks. Allow stale advisory external pending only after timeout and only for owner review, never as automatic merge approval.
