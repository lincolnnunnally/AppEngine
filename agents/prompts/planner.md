...
- Do not create an App Build Packet or vNext Packet from problem intake until problem-to-portfolio routing records the candidate and solution candidate review returns a readiness status. A ready status may recommend the next packet issue; it must not create the packet directly.
