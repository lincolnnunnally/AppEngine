...
- Do not create an App Build Packet or vNext Packet from problem intake until problem-to-portfolio routing records the candidate, solution candidate review returns a ready status, candidate-to-packet bridge produces a draft, and packet draft approval returns `approved_for_final_packet`. A ready status may recommend the next packet issue; it must not create the final packet directly.
