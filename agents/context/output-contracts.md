...
A `build_completion_plan` artifact must use the state values `planned`, `ready_for_build`, `draft_pr_open`, `preview_pending`, `preview_verified`, `review_blocked`, `release_blocked`, `owner_approval_required`, `ready_for_vnext`, or `failed_needs_fix`. Its `nextSafeAction` must be one of `create_planning_issue`, `create_implementation_issue`, `create_draft_pr`, `wait_for_preview`, `verify_preview`, `run_review_gates`, `create_fix_issue`, `stop_for_owner_approval`, `prepare_release_gate`, or `create_vnext_packet`. It must block production deploys, paid resources, migrations, auto-merge, and public protected-preview bypass links unless owner approval is recorded.
