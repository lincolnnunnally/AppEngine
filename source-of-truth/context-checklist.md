...
- App Engine Soft-Launch Readiness when Step 4 touches appengine.unitedundergod.org, both entry doors, health checks, owner login, provider/spend guardrails, or controlled deploy readiness. Historical artifact kind: `we_succeed_soft_launch_readiness`. we-succeed.org is not the App Engine Step 4 target.
...
- If this touches Step 4 for AppEngine/Opportunity, does a `we_succeed_soft_launch_readiness` artifact (historical identifier for App Engine Step 4 soft-launch readiness) lock the target to `https://appengine.unitedundergod.org`, require `/api/health`, verify both doors end to end, require owner login, provider/spend guardrail evidence, release gate evidence, rollback notes, and owner notes, and avoid ChurchConnect/ecosystem app work and we-succeed.org as the App Engine host?
...
- If an `opportunity_clarification` is being routed, does an `opportunity_solution_path` artifact name recommended path, reason, first practical step, needed resources, blockers, confidence, next AppEngine prompt, and no-build/no-Codex guardrails?
