...
### STEP 5 — first real problem THROUGH AppEngine  🔵 IN REVIEW (AppEngine #194 open; ChurchConnect #9 merged; deploy evidence pending)
- 🔵 **5 · ChurchConnect visitor bug as `extend_existing` → vNext → Codex in the ChurchConnect repo** (gated AppEngine proof only). The verify-after-publish walkthrough is built and proven here, on the existing Reviewer/Tester pieces — never recreated.
  done so far: ✅ AppEngine side (**#191**, merged) — RUN-001 carried through the existing pipeline; prior-work verdict `extend_existing` → materialized the vNext packet + follow-ups (8 phases). This proves the pipeline can route a real problem to executable work. Step 5 is not done until the ChurchConnect repo is coded, migrated to the Supabase path, deployed within configured limits, and verified live.
  ✅ **direction correction (Lincoln, 2026-06-25; AppEngine alignment PR #194):** ChurchConnect recovery must prove AppEngine can execute a real ecosystem-app improvement. It is **not** parked as unrelated side work. Code execution still happens in the ChurchConnect repo, but the work must trace back to this Step 5 packet/run record, acceptance criteria, review gate, and verify-after-publish evidence.
  ✅ **data-layer target:** pull ChurchConnect away from Mongo/Emergent and place the operational care/connect workflows on the consolidated Life Produces Life / ChurchConnect Supabase spine. Treat Mongo/Emergent branches as source material only; do not merge Mongo launch work as the destination. The first proof should map visitor follow-up onto existing Supabase tables such as `people`, `guests`, `guest_followup_tasks`, `care_requests`, and `care_follow_ups`, adding a new inbox table only if the gate proves the existing tables are insufficient.
  ✅ **ChurchConnect execution merged:** **ChurchConnect #9** (`f2354ccba6b61dac16d27c143c236d1eefff4927`) routes visitor registration through a backend Supabase path, writes to `people` / `guests` / `guest_followup_tasks`, keeps the service role key server-side, and documents the backend Supabase URL env needed for deploy. Live read-only Supabase proof confirmed the target tables exist and visitor/follow-up proof tables are empty before the walkthrough.
  remaining for ✅: merge AppEngine alignment **#194** as appropriate, configure existing backend deployment env (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`), deploy/publish within configured limits, verify the live visitor-follow-up workflow end to end, prove health check, record walkthrough evidence back into AppEngine, then mark Step 5 done. Step 6 stays blocked until that evidence exists.
  owner: AppEngine Step 5 loop (Codex/Claude Code executing in ChurchConnect repo) · Lincoln (launch/data decisions)
...
Architect/Source-of-Truth gate (Agent #4, to be built into the engine) keep effort on the finish line. Check the ledger. Claim your item. Build forward. Never recreate what's listed DONE.
