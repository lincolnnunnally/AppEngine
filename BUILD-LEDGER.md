...
### STEP 5 — first real problem THROUGH AppEngine  🔵 IN REVIEW (AppEngine #194 + ChurchConnect #9 open; deploy evidence pending)
- 🔵 **5 · ChurchConnect visitor bug as `extend_existing` → vNext → Codex in the ChurchConnect repo** (gated AppEngine proof only). The verify-after-publish walkthrough is built and proven here, on the existing Reviewer/Tester pieces — never recreated.
  ...
  🔵 **ChurchConnect execution PR open:** **ChurchConnect #9** routes visitor registration through a backend Supabase path, writes to `people` / `guests` / `guest_followup_tasks`, keeps the service role key server-side, and documents the backend Supabase URL env needed for deploy. Live read-only Supabase proof confirmed the target tables exist and visitor/follow-up proof tables are empty before the walkthrough.
  remaining for ✅: merge AppEngine alignment **#194** and ChurchConnect execution **#9** as appropriate, configure existing backend deployment env (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`), deploy/publish within configured limits, verify the live visitor-follow-up workflow end to end, prove health check, record walkthrough evidence back into AppEngine, then mark Step 5 done. Step 6 stays blocked until that evidence exists.
