...
- 🟡 **5 · ChurchConnect visitor bug as `extend_existing` → vNext → Codex in the ChurchConnect repo** (gated AppEngine proof only). The verify-after-publish walkthrough is built and proven here, on the existing Reviewer/Tester pieces — never recreated.
  ...
  ✅ **direction correction (Lincoln, 2026-06-25):** ChurchConnect recovery must prove AppEngine can execute a real ecosystem-app improvement. It is **not** parked as unrelated side work. Code execution still happens in the ChurchConnect repo, but the work must trace back to this Step 5 packet/run record, acceptance criteria, review gate, and verify-after-publish evidence.
