...
- 🟡 **5 · ChurchConnect visitor bug as `extend_existing` → vNext → Codex in the ChurchConnect repo** (gated AppEngine proof only). The verify-after-publish walkthrough is built and proven here, on the existing Reviewer/Tester pieces — never recreated.
  done so far: ✅ AppEngine side (**#191**, merged) — RUN-001 carried through the existing pipeline; prior-work verdict `extend_existing` → materialized the vNext packet + follow-ups (8 phases). This proves the pipeline can run a real problem to a handoff. Fail-closed, nothing deployed.
  ...
  remaining for ✅: execute the ChurchConnect visitor-follow-up recovery in `github.com/lincolnnunnally/ChurchConnect` from the AppEngine Step 5 packet; verify the app uses the Supabase path, runs the visitor-follow-up workflow end to end, publishes/deploys only inside configured limits, and records live walkthrough evidence back into AppEngine before Step 6 opens.
