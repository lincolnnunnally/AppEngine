...
- 🟡 **Ops attention queue: "needs my attention" panel with directed actions** — CLAIMED (Claude Code, 2026-07-03, branch `feat/ops-attention-queue`). Per app: reachability/health of the live URL, missing required env-var NAMES (generated apps publish `requiredEnvironment`; Vercel API lists which names are set — never values), still-on-`*.vercel.app` = needs a domain, stats-endpoint adoption. Rolls up into one sorted attention panel on owner-control where every item is a concrete action sentence. Lincoln may reorder (revenue deep-dive / send-to-Claude-Code button are the other staged items).
