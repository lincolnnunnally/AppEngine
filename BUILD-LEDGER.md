...
- 🔵 **URL status board: every app's domain situation, at a glance** — IN REVIEW (**#275**), Claude Code (Fable), 2026-07-03, branch `claude/ops-url-status` (Lincoln-directed 2026-07-03). Every app in `source-of-truth/ecosystem-portfolio-registry.json` shows one of LIVE at its domain · DEPLOYED awaiting domain (`*.vercel.app`-style host only) · DOMAIN OWNED but nothing serving · AWAITING URL — each with the concrete next step — on the owner dashboard. Owner-provided domain facts (2026-07-03) stored per app as a registry `domain` block (standard amended in `source-of-truth/app-portfolio-registry.md`); milstead.us (community) vs milstead.church (Milstead Baptist Church) recorded as DIFFERENT apps. Display + registry data only — no DNS changes, no paid actions.
