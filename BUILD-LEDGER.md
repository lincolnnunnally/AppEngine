...
### STEP 5 — first real problem THROUGH AppEngine  🟡 IN PROGRESS (AppEngine-gated ChurchConnect recovery)
- 🟡 **5 · ChurchConnect visitor bug as `extend_existing` → vNext → Codex in the ChurchConnect repo** (gated AppEngine proof only). The verify-after-publish walkthrough is built and proven here, on the existing Reviewer/Tester pieces — never recreated.
  ...
  remaining for ✅: execute the ChurchConnect visitor-follow-up recovery in `github.com/lincolnnunnally/ChurchConnect` from the AppEngine Step 5 packet; code the Supabase integration, run the needed migrations inside the existing consolidated Supabase path, deploy/publish within configured limits, verify the visitor-follow-up workflow end to end on the live app, and record walkthrough evidence back into AppEngine before Step 6 opens.
