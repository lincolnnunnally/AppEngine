...
- ⛔ **T5 · ChurchConnect finish (first app through the transfer)** — prep COMPLETE 2026-07-03 (readiness package delivered; see `loop-runs/2026-07-03-churchconnect-readiness.md`): gate (c) PR cleanup DONE under Lincoln's "move forward" (ChurchConnect #11 transfer-ledger + #7 fan-out tests MERGED, #5 closed reopenably, #12 left to the active branding session). Gate (a) walkthrough ✅ **CROSSED 2026-07-03 at the API layer** (see `loop-runs/2026-07-03-churchconnect-walkthrough-proof.md`): Lincoln authorized using the seeded `Lincoln@milstead.church` staff account (its password = the existing super-admin env value); signed in on production, authenticated inbox GET returned 5 follow-ups incl. both June proof records, PATCH `f1abb897…` new→"contacted" (200), persistence re-verified. UI-layer sign-in was separately broken (stale `REACT_APP_BACKEND_URL` bundle bake) and fixed the same day via ChurchConnect PRs #13–#16 (other session, E2E-verified in a real browser). Remaining for Lincoln: ONLY gate (b) — the word "apply" for the 3-migration schema packet (slice v1.0 + fanout v1.0 + recommended publish_event REVOKE hardening; verified conflict-free against live, clean drop-schema rollback). Domain correction: **Live On Mission = live-on-mission.COM, LIVE** (301 → churchconnect.cloud/mission, a polished landing already inside ChurchConnect); the earlier ".org unregistered" note was based on Lincoln's slip, withdrawn.
