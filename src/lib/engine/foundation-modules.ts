...
function webhookApi(): GeneratedFile {
  return file("src/app/api/billing/webhook/route.ts", [
    ...
    "      // Store the currency Stripe reported so ops revenue is never a",
    "      // blind merge of mixed currencies (defaults to usd, the app's own).",
    '      const currency = object["currency"] ? String(object["currency"]).toLowerCase() : "usd";',
    "      await sql`insert into payments (email, amount_cents, currency, status) values (${email}, ${amount}, ${currency}, 'paid')`;",
    "    } catch {",
...
function adminStatsApi(): GeneratedFile {
  return file("src/app/api/admin/stats/route.ts", [
    ...
    "  }",
    "  if (!hasDatabase()) {",
    "    return NextResponse.json({ ok: true, reporting: false, users: null, ticketsOpen: null, ordersRecent: null, revenueCentsRecent: null, revenueCurrency: null, activity: null });",
    "  }",
    "  try {",
    "    const sql = getDatabase();",
    "    // Aggregates only: counts, a revenue SUM in minor units, and events per",
    "    // day — no names, emails, or line items ever leave the app. Revenue is",
    "    // grouped by currency so a mixed-currency table is reported as",
    "    // not-summable (null) rather than merged under one label. sum() is",
    "    // bigint: a 30-day cents total can exceed int4 (~$21.47M) and must not",
    "    // overflow and 500 the whole endpoint. Activity uses whole calendar",
    "    // days so the oldest day isn't a partial (rolling-window) count.",
    "    const [users, tickets, orders, revenueByCurrency, activity] = await Promise.all([",
    ...
    "      sql`select currency, coalesce(sum(amount_cents), 0)::bigint as n from payments",
    "          where status = 'paid' and created_at > now() - interval '30 days' group by currency`,",
    "      sql`select to_char(created_at, 'YYYY-MM-DD') as date, count(*)::int as count",
    "          from (select created_at from payments where created_at >= (current_date - interval '13 days')",
    "                union all",
    "                select created_at from support_tickets where created_at >= (current_date - interval '13 days')) events",
    "          group by 1 order by 1`",
    "    ]);",
    "    // One currency -> report it. Zero paid rows -> $0 in the app's own",
    "    // currency (usd by construction). More than one -> not summable.",
    "    const paidRows = revenueByCurrency.filter((row) => Number(row.n || 0) > 0);",
    "    const singleCurrency = paidRows.length <= 1;",
    "    const revenueCentsRecent = singleCurrency",
    "      ? Number((paidRows[0] || revenueByCurrency[0] || {}).n || 0)",
    "      : null;",
    "    const revenueCurrency = singleCurrency",
    "      ? String((paidRows[0] || revenueByCurrency[0] || {}).currency || \"usd\").toLowerCase()",
    "      : null;",
    ...
    "      revenueCentsRecent,",
    "      revenueCurrency,",
    "      activity: activity.map((row) => ({ date: String(row.date), count: Number(row.count || 0) })),",
...
export function foundationSchemaSql(): string {
  return [
    ...
    "  created_at timestamptz not null default now()",
    ");",
    "-- Self-applying for apps generated before the currency column shipped.",
