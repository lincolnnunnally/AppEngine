...
function adminStatsApi(): GeneratedFile {
  return file("src/app/api/admin/stats/route.ts", [
    ...
    "    // day — no names, emails, or line items ever leave the app.",
    "    const [users, tickets, orders, revenue, activity] = await Promise.all([",
    ...
    "          from (select created_at from payments where created_at > now() - interval '14 days'",
    "                union all",
    "                select created_at from support_tickets where created_at > now() - interval '14 days') events",
    ...
    "      revenueCentsRecent: Number(revenue[0] ? revenue[0].n || 0 : 0),",
    '      revenueCurrency: "usd",',
