...
function supportLibFile(): GeneratedFile {
  return file("src/lib/db/support.ts", [
    ...
    "  await forwardTicketToOwnerInbox(input).catch(() => undefined);",
    "  return true;",
    "}",
    "",
    "async function forwardTicketToOwnerInbox(input: { email: string; subject: string; body: string }): Promise<void> {",
    '  const inboxUrl = (process.env.APP_ENGINE_INBOX_URL || "").trim();',
    '  const token = (process.env.APP_ENGINE_INBOX_TOKEN || process.env.APP_ENGINE_STATS_TOKEN || "").trim();',
    "  if (!inboxUrl || !token) return;",
    "  await fetch(inboxUrl, {",
    '    method: "POST",',
    '    headers: { "content-type": "application/json", authorization: `Bearer ${token}` },',
    "    body: JSON.stringify({",
    '      app: process.env.APP_SLUG || process.env.VERCEL_PROJECT_PRODUCTION_URL || "generated-app",',
