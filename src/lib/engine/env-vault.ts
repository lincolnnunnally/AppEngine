...
export const KNOWN_KEYS: readonly KnownKey[] = [
  ...
  { key: "VERCEL_TOKEN", scope: "universal", engineRuntime: true, usedFor: "Attaching customer web addresses to apps and issuing their HTTPS certificates (one token covers every project in the team). Apps that read it under a different name — AI Website Design uses AWD_VERCEL_TOKEN, because Vercel reserves VERCEL_* for its own system variables — are mapped automatically; save it here ONCE.", whereToFind: "vercel.com → Account Settings → Tokens → Create Token, scope it to the life-produces-life team, and set expiry to No Expiration. ⚠️ Do NOT paste a token from `vercel login` on a laptop — those expire in about 12 hours and fail silently, leaving customers with web addresses that resolve but show a security warning." },
  { key: "WORDPRESS_URL", scope: "universal", usedFor: "The WordPress Multisite network customer websites are created on. This is the HANDOFF target: a site is built once by AI, pushed in as an editable Divi layout, and from then on the customer edits it in Divi's visual builder — it is never regenerated.", whereToFind: "The address of your Multisite install, e.g. https://sites.easypeazy.site. One-time setup is about 20 minutes: cPanel → Softaculous → WordPress, then Tools → Network Setup choosing SUB-DIRECTORIES (sub-domains need a second-level wildcard certificate that Cloudflare's free SSL does not cover)." },
  { key: "WORDPRESS_USER", scope: "universal", usedFor: "Network Super Admin the push authenticates as (needs rights to create customer subsites)", whereToFind: "The username of a Super Admin on your Multisite network" },
  { key: "WORDPRESS_APP_PASSWORD", scope: "universal", usedFor: "Authenticates the push to WordPress — NOT the login password", whereToFind: "WordPress → Users → your profile → Application Passwords → Add New. Copy it with or without spaces; both work." },
  { key: "ELEGANT_THEMES_USERNAME", scope: "universal", usedFor: "Licences and updates Divi on the network. One membership covers unlimited client sites.", whereToFind: "elegantthemes.com → Account" },
  { key: "ELEGANT_THEMES_API_KEY", scope: "universal", usedFor: "Paired with the Elegant Themes username to licence Divi", whereToFind: "elegantthemes.com → Account → API Key" },
  { key: "VERCEL_TEAM_ID", scope: "universal", usedFor: "Scopes Vercel API calls to your team. Without it a valid token is applied to a personal scope and your projects aren't found.", whereToFind: "vercel.com → your team → Settings → General → Team ID" },
