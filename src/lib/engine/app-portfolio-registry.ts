...
// included. The records themselves live in imported-ecosystem-apps.ts (data
// only, no engine dependencies) so lightweight public surfaces — the
// apps.unitedundergod.org showcase — can read the same roster without pulling
// this module's full import graph. These are seeded imports; completing any of
// them runs through the normal intake -> prior-work-check (extend_existing) ->
// build loop, and its entry then picks up live state.
