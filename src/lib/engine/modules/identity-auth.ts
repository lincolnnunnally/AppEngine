...
function sessionFile(): GeneratedModuleFile {
  return file("src/lib/auth/session.ts", [
    ...
    "",
    "export type CurrentUser = { name: string; email: string; role: Role; mode: \"session\" | \"setup\" };",
    "",
    ...
    '    return { name: "Local Setup User", email, role: "owner", mode: "setup" };',
    "  }",
    "  const session = await auth();",
    "  const email = session?.user?.email;",
    "  if (!email) return null;",
    '  const role = (session.user?.role as Role | undefined) ?? "customer";',
    "  return { name: session.user?.name || email, email, role, mode: \"session\" };",
    "}",
    "",
